...
class ID3DecisionTree:
    def __init__(self, p=0, q=0):
        ...
        self.p = p # probability of not splitting
        self.q = q # hyperparameter to control p's change by tree depth
    ...
    def fit(self, X, y):
        ...
        self.tree = self._build_tree(data, data.columns[:-1], level=0)
    ...
    def _print_tree_recursive(self, node, indent=''):
        ...
        for value, child_node in node.children.items():
            ...
            self._print_tree_recursive(child_node, indent + '  ')

    def _build_tree(self, data, attributes, level):
        ...
        # calculate p for the current tree level
        p_level = self.p * (1 + self.q) ** level
        ...
        for value in data[best_attr].unique():
            ...
            elif random.random() <= p_level:
                node.children[value] = Node(label=subset['label'].value_counts().idxmax())
            else:
                node.children[value] = self._build_tree(subset, [attr for attr in attributes if attr != best_attr], level+1)
